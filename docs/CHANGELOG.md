...
- [改进] 手机 App 问股与 AI 分析等待态补充已用时、阶段进度、慢响应提示和分析取消中止轮询，降低模型响应较慢时的等待不确定感。
- [改进] 手机 App BFF 将行情短缓存调整为 60 秒并为股票搜索增加 60 秒内存缓存，减少页面切换和重复搜索时的外部请求。
- [改进] 手机 App 问股策略收敛为 6 个主要策略并对齐后端真实策略 ID，默认改走普通问股接口以避免流式失败后重复请求和额外 token 消耗。
- [改进] 手机 App 问股新增快速/深度模式切换，快速模式仅暴露实时行情、历史 K 线、趋势和均线工具并注入快速模式提示，减少慢数据源调用和无效工具尝试；深度模式保留完整 Agent 工具链。
- [文档] 新增手机 App 当前关键问题记录，整理快速问股仍偏慢、自选 AI 分析报告慢、美股行情源失败和 SSE 暂缓等后续待办。
- [文档] 新增手机版 App 综合评估报告，覆盖 UI/UX、功能、性能、兼容性、稳定性、安全性、可访问性和测试质量。
- [改进] 手机 App 自选股卡片改为进入个股详情页，并优化详情页行情展示、AI 深度分析入口、快速问股入口和长耗时提示。
- [改进] 手机 App AI 分析等待页新增“后台继续，返回自选”和“查看历史记录”入口，降低长报告生成时的等待束缚感。
- [改进] 手机 App 个股详情页的“快速问股”现在会自动带入股票上下文并预填问题；自选卡片文案同步为详情/分析/问股入口。
- [修复] 手机 BFF 批量行情修复新浪 A 股无效 0 元行情被当作成功结果的问题，并补充 GBK 解码，避免自选股 A 股卡片价格/涨跌幅固定异常。
- [改进] 手机 App 接入全局 ErrorBoundary，避免页面运行时异常直接表现为白屏，并提供异常提示和重试入口。
- [改进] 自选页自动刷新改为 60 秒静默刷新，增加“刚刚更新/静默更新/显示缓存”轻量状态提示，并在搜索弹窗打开时暂停自动刷新。
- [修复] 自选页主列表由 FlatList 改为 ScrollView 渲染，规避 React Native Web 自动刷新时偶发 removeChild 节点异常。
- [修复] 自选页 Web 端禁用 ScrollView 的 RefreshControl，改用顶部“刷新”按钮，解决 RefreshControl 在 React Native Web 上触发 removeChild 异常。
- [改进] 关于/反馈页移除模拟反馈提交和占位开源地址，明确提示反馈后端暂未接入，并修正美股支持文案。
- [改进] 自选页以自选代码为准保留行情失败股票卡片，并在添加股票弹窗显示“已添加”和搜索无结果提示。
- [改进] 手机 BFF 清理港股行情调试日志，并对当前暂不支持的美股代码跳过共享后端回退，减少 AAPL/TSLA 日志刷屏。
- [改进] 大盘页拆分“刷新数据”和“生成 AI 大盘复盘”，下拉刷新不再触发 AI 长任务，并优化生成中状态和占位文案。
- [修复] 大盘复盘完成状态使用通用任务轮询，避免任务已完成但按钮仍显示生成中。
- [改进] 自选股卡片新增右上角跨端操作菜单，支持查看详情、AI 深度分析、快速问股和删除自选，并避免 Web 端系统 Alert/长按兼容问题。
- [改进] 问股页错误提示改为用户语言，避免直接暴露 HTTP/API 地址等技术信息。
- [改进] 设置页将后端地址、BFF 和 API Key 折叠到开发者设置，并增加共享后端/BFF 连接测试。
- [改进] 移除设置页重复的分析历史入口，历史分析统一从“我的页”进入。
- [改进] 历史记录页清空历史改用批量删除，并补充清除中状态和失败提示；删除/清空确认改为 Web 可用的自定义弹层。
- [改进] ErrorBoundary 改为面向用户的通用异常提示，并清理自选页临时调试轨迹和 removeChild 调试文档。
